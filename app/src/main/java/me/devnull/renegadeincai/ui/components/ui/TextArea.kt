...
fun TextArea(
    ...
    val importSuccessMessage = stringResource(R.string.text_area_import_success)
    val importFailedMessage = stringResource(R.string.text_area_import_failed)
        ...
        uri?.let {
            scope.launch {
                try {
                    ...
                    toaster.show(importSuccessMessage, type = ToastType.Success)
                } catch (e: Exception) {
                    e.printStackTrace()
                    val errorMessage = e.message ?: importFailedMessage
