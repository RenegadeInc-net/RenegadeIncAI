    ...
    fun saveFile(relativePath: String, content: String, onResult: (String?) -> Unit) {
        viewModelScope.launch(Dispatchers.IO) {
            if (relativePath == "SKILL.md") {
                ...
                if (name != skillName) {
                    withContext(Dispatchers.Main) { onResult(application.getString(me.devnull.renegadeincai.R.string.skill_detail_vm_name_change_disallowed, skillName)) }
                    return@launch
                }
            }
            val success = skillManager.saveSkillFile(skillName, relativePath, content)
            loadFiles()
            withContext(Dispatchers.Main) { onResult(if (success) null else application.getString(me.devnull.renegadeincai.R.string.skill_detail_vm_save_failed)) }
        }
    }
