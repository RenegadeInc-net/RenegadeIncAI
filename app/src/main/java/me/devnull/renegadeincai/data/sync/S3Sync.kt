...
import me.devnull.renegadeincai.data.files.SkillPaths
import me.devnull.renegadeincai.data.datastore.Settings
import me.devnull.renegadeincai.data.datastore.SettingsStore
import me.devnull.renegadeincai.data.datastore.migration.SettingsJsonMigrator
import me.devnull.renegadeincai.data.sync.s3.S3Client
...
class S3Sync(
    ...
    suspend fun backupToS3(config: S3Config) = withContext(Dispatchers.IO) {
        ...
        val client = getS3Client(config)
        val key = "renegadehub_backups/${file.name}"
    ...
    suspend fun listBackupFiles(config: S3Config): List<S3BackupItem> = withContext(Dispatchers.IO) {
        ...
        val result = client.listObjects(
            prefix = "renegadehub_backups/",
            maxKeys = 1000
        ).getOrThrow()

        result.objects
            .filter { it.key.startsWith("renegadehub_backups/backup_") && it.key.endsWith(".zip") }
            .map { obj ->
                S3BackupItem(
                    key = obj.key,
